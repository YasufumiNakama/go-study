...
func Example_structAsKey() {
	...
	Display("money", money)
	// Output:
	// Display money (map[display.Movie]int):
	// money[display.Movie{Title: "movie1", Yesr: 1996}] = 1000
	// money[display.Movie{Title: "movie2", Yesr: 2022}] = 3000
}

func Example_arrayAsKey() {
	city := [2]string{"Saitama", "Shiki"}

	money := map[[2]string]int{
		city: 1000,
	}

	Display("money", money)
	// Output:
	// Display money (map[[2]string]int):
	// money[[2]string{"Saitama", "Shiki"}] = 1000
}
...
func Example_recursive() {
	...
	// (*c.Tail).Value = 1
	// (*(*c.Tail).Tail).Value = 1
	// (*(*(*c.Tail).Tail).Tail).Value = 1
	// (*(*(*(*c.Tail).Tail).Tail).Tail).Value = 1
	// (*(*(*(*(*c.Tail).Tail).Tail).Tail).Tail).Value = 1
	// (*(*(*(*(*(*c.Tail).Tail).Tail).Tail).Tail).Tail).Value = 1
	// (*(*(*(*(*(*(*c.Tail).Tail).Tail).Tail).Tail).Tail).Tail).Value = 1
	// (*(*(*(*(*(*(*(*c.Tail).Tail).Tail).Tail).Tail).Tail).Tail).Tail).Value = 1
	// (*(*(*(*(*(*(*(*(*c.Tail).Tail).Tail).Tail).Tail).Tail).Tail).Tail).Tail).Value = 1
}

...
func (s *IntSet) Len() int {
	...
	for _, word := range s.words {
		...
		if UINT_SIZE == 32 {
			len += bits.OnesCount32(uint32(word))
		} else {
			len += bits.OnesCount64(uint64(word))
		}
	}
	return len
}
...
func (s *IntSet) Copy() *IntSet {
	var c IntSet
	c.words = make([]uint, len(s.words)) // make([]uint64, len(s.words))
...
func (s *IntSet) Elems() []int {
	...
	for i, word := range s.words {
		...
		for bit := 0; bit < UINT_SIZE; bit++ {
			if word&(1<<uint(bit)) != 0 {
				elems = append(elems, UINT_SIZE*i+bit)
			}
		}
	}
	return elems
